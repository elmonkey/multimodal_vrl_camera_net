...
class thermal_camera():
    ...
    def send_msg(self, bmRequestType, bRequest, wValue=0, wIndex=0, data_or_wLength=None, timeout=None):
        assert (self.therm.ctrl_transfer(bmRequestType, bRequest, wValue, wIndex, data_or_wLength, timeout) == len(data_or_wLength))
    ...
    def __init__(self):
        ...
        if not self.therm: raise ValueError('Device not found')
        ...
        ep = usb.util.find_descriptor(cfg[(0,0)], custom_match=custom_match)   # match the first OUT endpoint
...
#        brightness = 0
#        scale = 0
#        factor = 255/float(total_pixels)
#        for n in range(num_bins+1):
#            cur_bin = low+n*bin_size
#            mask = cv2.inRange(frame,cur_bin,(cur_bin+bin_size-1))
#            mask >>= 7
#            mask_hold += mask
#            brightness += scale
#            scale = (cv2.countNonZero(mask)*factor)
#            temp = ((frame-cur_bin)/bin_size)*scale + brightness
##            temp = mask*brightness
#            output += np.multiply(temp,mask)
